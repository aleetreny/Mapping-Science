...
The thesis snapshot is the processed text corpus, paired with the active SPECTER2 embedding set. These implementation-level names are kept here so that the main chapters can remain focused on methodology rather than repository layout.

\medskip

Table~\ref{tab:pipeline_stages} summarizes the sequential pipeline stages, the main scripts that execute each stage, and the primary outputs used by downstream analysis. All scripts are located in the repository \texttt{scripts/} directory and are designed to run in order. Configuration parameters such as sampling targets, year ranges, and metric hyperparameters are set in \texttt{config.yaml}.
...
\end{tabular}
\par\vspace{0.15cm}
\footnotesize\textit{Note.} Script names refer to files in the \texttt{scripts/} directory. UMAP visualization scripts (09, 10, 10b, 10c) run between stages 5 and 6 but produce outputs not used for quantitative analysis.
\end{table}
