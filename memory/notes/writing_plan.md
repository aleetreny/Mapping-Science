...
- **Exploratory Typologies**: Unsupervised clustering is strictly exploratory and must be conducted *only* on the eight structural embedding-space metrics, not on UMAP coordinates. The clusters must be interpreted as descriptive typologies (conceptual heuristics) rather than absolute natural categories of science.
...
| **09 Typologies** | Unsupervised clustering of subfields using the eight-metric structural profiles to discover descriptive typologies of disciplines. | `scripts/18_explore_morphological_typologies.py` | `outputs/04_reduced_metric_core/reduced_interpretable_core_metrics.parquet` | Clustering dendrograms and cluster member profile tables.<br>Integrated UMAP plots for cluster visualization. |
...
- **Draftable Immediately** (Do not depend on final rerun metrics):
  ...
  - **Chapter 5: Embedding-Space Morphological Metrics** — Mathematical equations and definitions for the eight structural metrics are fully defined and can be written now.
- **Requires Final Pipeline Run Outputs** (Depend on final script execution):
  ...
  - **Chapter 9: Exploratory Morphological Typologies** — Built from the eight-metric structural profiles.
